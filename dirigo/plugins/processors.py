...
def dewarp_kernel(
    ...
    for si in prange(Ns): # si = Slow pixel Index
        ...
        ri = 0 + si // Ndirections # record index (for unidirectional scanning ri=si)
...
class RasterFrameProcessor(Processor):
    ...
    def run(self):
        ...
        while True: # Loops until receives sentinel None
            ...
            dewarped = dewarp_kernel(buf.data, self.dewarped_shape, start_indices, nsamples_to_sum)
            self.publish(dewarped) # sends off to Logger or Display workers
